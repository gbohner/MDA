function data = exporttoexcel( AvgedImages, filename )
  ...
  for j1 = 1:size(AvgedImages,1)   
    ...
    try
      ...
      if strcmp(AvgIm.mt.model,'u')
        ...
        %Lateral
      x(end+1) = (AvgIm.mt.fit.x(:).value - AvgIm.gfp.fit.x(:).value)' * ...
        [-sin(AvgIm.mt.fit.o.value); cos(AvgIm.mt.fit.o.value)] * ...
        Config.analysis.getgfpdata.croppixelsize;
    catch
    end
...
for j1 = 1:1
  xlswrite(filename,data{j1}, ['Results' num2str(j1)], ['A2:' get_excel_column(size(data{j1},2)) num2str(size(data{j1},1)+1)]);
end
  ...
  function WriteNames(filename, sheet_num)
     %%% General stuff
      ...
      names{end+1} = 'Lateral posdiff (nm)';
...
% try
   exporttoexcel_linescans(AvgedImages, filename);
% catch
