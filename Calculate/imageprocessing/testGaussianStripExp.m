function [ f, xb ] = testGaussianStripExp( x,xg,yg, offset, sf )
...
%   baseintensity = exp(-((sin(x(3))*offset)^2 + (cos(x(3))*offset)^2) / x(4));

  rad = -x(3);
  scaleMatrix = [sf * cos(rad)^2 + 1 * sin(rad)^2,  1 * cos(rad) * sin(rad) - sf * cos(rad) * sin(rad) ;...
