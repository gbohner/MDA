...
%      
  ...
  for i = 1:num_points
    switch modelstr(i)
      ...
      case 'c'
        fit_model{i} = Model2DComet( guess(i) );
      case 'u'
        fit_model{i} = Model2DExponentiallyModifiedGaussian( guess(i) ); %Good for comet too with Tau<Inf
      case 'v'
        fit_model{i} = Model2DEMG_lattice( guess(i) );
      case 'a'
        fit_model{i} = Model2DGaussStrechedAsymmetric( guess(i) ); %gauss with lattice binding;
        
      % elongated objects
      case 'e'
        fit_model{i} = Model2DFilamentEnd( guess(i) );
      case 's'
        fit_model{i} = Model2DFilamentEndGaussSurv( guess(i) );
      case 'm'
...
%      disp('Error in fitting. Continueing');
  end
...
%   %%%%%%%%%%%%%%%%%
%     myfilename = ['C:\debug\06myjacobians' num2str(numel(szamlal)) '.mat'];
%     save(myfilename, 'jacobian');
%     %%%%%%%%%%
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %  
%       if strcmp(modelstr,'e') || strcmp(modelstr,'u')
%         global CoDlist fitpiclist xlist;
%         CoDlist = [CoDlist CoD];
%         fitpiclist{end+1} = reshape(fit_pic,17,17);
%         xlist = [xlist; x(2:3)];
%         save 'C:\debug\Codlist.mat' 'CoDlist'
%         save 'C:\debug\fitpiclist.mat' 'fitpiclist'
%         save 'C:\debug\xlist.mat' 'xlist'
%       end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %  
%        global Chilist;
%        Chilist = [Chilist reduced_chi]; 
%        save 'C:\debug\Chilist.mat' 'Chilist'
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
%     options.PlotFcns = {@optimplotresnorm @optimplotfval @optimplotstepsize @optimplotfirstorderopt};
%     pause(5);
    % set used methode
...
%      numvalt(mivalt~=x)=numvalt(mivalt~=x)+1;
%      mivalt=x;
      f = fit_pic - x(1); % first subtract background
      % than subtract contribution of each model
      for i_m = 1:numel(fit_model)
...
%       global h2;
%       figure(h2); imagesc(reshape(evaluate( fit_model{i_m}, x(ids(i_m):ids(i_m+1)-1)),17,17));
      
%      
% finalerror(numel(szamlal)) = sum(sum(f));
%       
