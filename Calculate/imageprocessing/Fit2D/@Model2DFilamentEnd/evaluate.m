function [ f, xb ] = evaluate( model, x )
  ...
  if nargout == 1 % calculate value of function
    ...
    f = x(5) * ( w .* ( exp( -0.5*( (xg-x(1)).^2 + (yg-x(2)).^2 ) / x(4).^2 ) ) + ...
               ( 1 - w ) .* GaussianStripSquaredSigma( x(1:4) ) );
    ...
    [ g, gb ] = GaussianStripSquaredSigma( x(1:4) );

    % forward calculation
    temp = ( ( xg-x(1) ).^2 + ( yg-x(2) ).^2 ) ./ x(4).^2;
    temp0 = exp( -0.5* temp );
    tempb = -( w .* temp0 .* x(5) ./ x(4).^2 );
