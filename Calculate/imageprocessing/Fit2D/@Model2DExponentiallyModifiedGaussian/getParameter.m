function [ model, x0, dx, lb, ub ] = getParameter( model, data )
  ...
  if isempty( model.guess.l )
      try
          global Gui
          if Gui.state.tracking
            model.guess.l = [3000000 3000000];
          else
            model.guess.l = [30 100];
          end
      end
  end
  ...
  model.guess.h = model.guess.l(2) * model.guess.h;
  
  
  % setup parameter array
  %    [ X  Y           Orientation     Sigma_PSF  w         Height (A)       Tau]
  x0 = [ c(1:2)         model.guess.o   model.guess.w(1:2)   model.guess.h    model.guess.l(1) ];
  dx = [ 1  1           1e-8            [0.1 0.1]            10               0.1              ];
  lb = [ 1  1           0               [1 1]                1                5                ];
  ub = [ data.rect(3:4) 6.3             [50 50]              Inf              Inf              ];
