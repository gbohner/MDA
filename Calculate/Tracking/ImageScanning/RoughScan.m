function [ objects, params ] = RoughScan( orig_pic, pic, bw )
...
global roughpic
  ...
  params.background = mean( orig_pic( bw == 0 & ~isnan(orig_pic) ) ) ;
...
background = imopen(orig_pic ,strel('disk',15));
for i74 = 1:numel(objects)
      obp = objects(i74).p;
      for i741 = 1: numel(obp)
        obp(i741).b = background(round(obp(i741).x(2)),round(obp(i741).x(1)));
      end
      objects(i74).p = obp;
end
% % % % % % % % % % % % % % % % % % % % %   
...
% find the local maxima area in the right area
  ...
  % sort maxima by their intensity
  ...
  if params.max_beads_per_region > 1 && size(maximas,1) > 1 % many maxima in the region    
    ...
    idx = getClusters( maximas, 1, 2 ); % find close points
    for i = unique( idx )
      f = find( idx == i );
      maximas( f(2:end), 3 ) = -1;
    end
...
%   color_idx = 1;
  ...
  image(2:end-1,2:end-1) = roughpic(region_stats.BoundingBox(2)+0.5:region_stats.BoundingBox(2)+region_stats.BoundingBox(4)-0.5,...
...
% hold on;
% scatter(ex+region_stats.BoundingBox(1), ey+region_stats.BoundingBox(2), 'r');
...
%       PlotOrientations( a( :, [1 2 4 ] ) );
  ...
  for i = 1:numel(chains) % run through chains
    ...
    if size( chains{i}, 1 ) > 1 % elongated object
      if size( chains{i}, 1 ) < 1 %params.short_object_threshold
      ...
      for k = 1:size( chains{i}, 1 ) % run through all points of chain
        ...
        objects(i).p(k).w = ( params.fwhm_estimate / 2.77258872223978 )^2;
        objects(i).p(k).b = NaN;
